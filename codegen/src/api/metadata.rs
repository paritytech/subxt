...
fn get_variant_uid(
    ...
    for field in var.fields() {
        bytes.extend(get_field_uid(registry, field, set));
    }
    ...
    let data = match ty_def {
        ...
        TypeDef::Variant(variant) => {
            ...
            for var in variant.variants() {
                bytes.extend(get_variant_uid(registry, var, set));
            }
            bytes
        }
...
fn get_type_uid(
    ...
    bytes.extend(get_type_def_uid(registry, ty_def, set));
...
pub fn get_metadata_uid(metadata: &RuntimeMetadataLastVersion) -> [u8; 32] {
