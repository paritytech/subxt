...
pub type MethodResponse = Result<Box<RawValue>, Error>;

/// The response back from an RPC subscription call.
pub type SubscriptionResponse = Result<(Box<RawValue>, mpsc::UnboundedReceiver<Box<RawValue>>), Error>;
...
impl BackgroundTaskHandle {
    ...
    pub async fn request(&self, method: String, params: Option<Box<RawValue>>) -> MethodResponse {
        ...
        self.to_backend.send(Message::Request { method, params, sender: tx });

        match rx.await {
            Err(_e) => Err(Error::SmoldotClosedUnexpectedly),
            Ok(response) => response
        }
    }

    /// Subscribe to some RPC method via the background task.
    pub async fn subscribe(&self, method: String, params: Option<Box<RawValue>>, unsubscribe_method: String) -> SubscriptionResponse {
        let (tx, rx) = oneshot::channel();
        self.to_backend.send(Message::Subscription { method, params, unsubscribe_method, sender: tx });
...
impl<TPlatform: PlatformRef, TChain> BackgroundTask<TPlatform, TChain> {
    ...
    pub(crate) fn new(
        ...
        let bg_handle = BackgroundTaskHandle {
            to_backend: tx
        };
...
struct ActiveSubscription {
    ...
    notification_sender: mpsc::UnboundedSender<Box<RawValue>>,
    /// The unsubscribe method to call when the user drops the receiver
    /// part of the channel.
    unsubscribe_method: String,
}
        ...
        match message {
            Message::Request {
                ...
                if let Err(err) = result {
                    ...
                    if sender
                        .send(Err(Error::RpcRequest(err.to_string())))
                        .is_err()
                    {
                        tracing::warn!(
                            target: LOG_TARGET,
            ...
            Message::Subscription {
                ...
                self
                    ...
                    .insert(id, pending_subscription);
        ...
        match RpcResponse::from_str(&response) {
            Ok(RpcResponse::Error { id, error }) => {
                ...
                if let Some(sender) = self.requests.remove(&id) {
                    if sender
                        .send(Err(Error::RpcRequest(error.to_string())))
                        .is_err()
                    {
                        tracing::warn!(
                            target: LOG_TARGET,
            ...
            Ok(RpcResponse::Method { id, result }) => {
                ...
                {
                    let Ok(sub_id) = serde_json::from_str(result.get()) else {
                        tracing::warn!(
                            target: LOG_TARGET,
                    ...
                    if pending_subscription.response_sender.send(Ok((result, sub_rx))).is_err() {
                        tracing::warn!(
                            target: LOG_TARGET,
                            "Cannot send method response to id={id} chain={chain_id:?}",
                        );
            ...
            Ok(RpcResponse::Subscription { method, id, result }) => {
                let Some(active_subscription) = self.subscriptions.get_mut(&id) else {
                    tracing::warn!(
                        target: LOG_TARGET,
                ...
                if active_subscription.notification_sender.send(result).is_ok() {
                ...
                let Some(active_subscription) = self.subscriptions.remove(&id) else {
                ...
                let request = format!(
                    r#"{{"jsonrpc":"2.0","id":"{}", "method":"{}","params":["{}"]}}"#,
                    unsub_id, active_subscription.unsubscribe_method, id
                );

                if let Err(err) = self.client.json_rpc_request(request, chain_id) {
                    tracing::warn!(
                        target: LOG_TARGET,
                        "Failed to unsubscribe id={id:?} chain={chain_id:?} method={:?} err={err:?}", active_subscription.unsubscribe_method
                    );
                } else {
                    tracing::debug!(target: LOG_TARGET,"Unsubscribe id={id:?} chain={chain_id:?} method={:?}", active_subscription.unsubscribe_method);
                }
            }
            Err(err) => {
