...
So, if you want to substitute in Substrate types, wrap them in `::subxt::utils::Static` in the type substitution, as above. [#886](https://github.com/paritytech/subxt/pull/886) also generally improves type substitution so that you can substitute the generic params in nested types, sicne it's required in the above.

Several types have been renamed as a result of this unification. Additionally, to obtain the bytes from a storage address, instead of doing:
...
Previously we'd only close the subscription and stop waiting when we saw a `Finalized` or `FinalityTimeout` event. Thanks for spotting this [@furoxr](https://github.com/furoxr)!
