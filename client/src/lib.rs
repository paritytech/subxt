...
impl SubxtClient {
        ...
        T: DeserializeOwned,
    {
        ...
        let msg = serde_json::to_string(&RequestSer::new(Id::Number(id), method, params))
            .map_err(JsonRpseeError::ParseError)?;
        self.to_back
            .clone()
...
fn read_jsonrpc_response(
    ...
    let res = match serde_json::from_str::<Response<JsonValue>>(&msg) {
        Ok(rp) if rp.id == id => Some(Ok(rp.result)),
        Ok(_) => Some(Err(JsonRpseeError::InvalidRequestId)),
        Err(_) => {
            match serde_json::from_str::<InvalidRequest<'_>>(&msg) {
                Ok(err) => {
                    let err = RpcError {
                        jsonrpc: TwoPointZero,
                        error: ErrorCode::InvalidRequest.into(),
                        id: err.id,
                    };
