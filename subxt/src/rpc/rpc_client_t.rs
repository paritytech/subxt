...
pub type RpcResponse<'a> = Pin<Box<dyn Future<Output = Result<Box<RawValue>, RpcError>> + Send + 'a>>;

/// The response returned from our [`RpcClientT`] implementation's `subscribe` method.
pub type RpcSubscription<'a> = Pin<Box<dyn Future<Output = Result<RpcSubscriptionStream, RpcError>> + Send + 'a>>;

/// The inner subscription stream returned from our [`RpcClientT`]'s `subscription` method.
pub type RpcSubscriptionStream = Pin<Box<dyn Stream<Item = Result<Box<RawValue>, RpcError>> + Send + 'static>>;
