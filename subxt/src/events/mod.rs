...
//! Parse the events generated by a block.
//!
//! This module exposes the ability to work with events generated by a given block.
//! There are two types of events that the subxt is internally aware of:
//! - static events: Events that are already present in the subxt's client metadata
//! - dynamic events: Events that are present only in the target node's metadata
//!
//! For the later case can happen when the subxt connects first to a target node,
//! then the target node performs a runtime upgrade. To mitigate this behavior and keep the
//! subxt's metadata in sync with the target node, please refer to [crate::updates::UpdateClient].
//!
