...
//! ### Acquiring metadata
//!
//! The simplest way to do acquire metadata is to use the `subxt` CLI tool to download it from your
...
//! ```
//!
//! Once installed, run `subxt metadata > metadata.scale` to download the node's metadata and save
//! it to some file (again, this assumes that the node you'd like to talk to is running locally; run
//! `subxt metadata --help` to see other options).
//!
//! Sometimes it's useful to be able to manually output the code that's being generated by the
//! [`#[subxt]`](crate::subxt) macro to better understand what's going on. To do this, use `subxt
//! codegen | rustfmt > generated_interface.rs`.
//!
//! ### Configuring the client
//!
//! To use Subxt, you must instantiate the client (typically an
//! [`OnlineClient`](crate::client::OnlineClient)) with some [`Config`](crate::config::Config).
//! Subxt ships with these default configurations:
//!
//! - [`crate::config::SubstrateConfig`] to talk to generic Substrate nodes.
//! - [`crate::config::PolkadotConfig`] to talk to Polkadot nodes.
//!
//! The configuration defines the types used for things like the account nonce, account ID and block
//! header, so that it can encode or decode data from the node properly. It also defines how
//! transactions should be signed, how data should be hashed, and which data should be sent along
//! with an extrinsic. Thus, if the configuration is incorrect, Subxt may run into errors performing
//! various actions against a node.
//!
//! In many cases, using one of the provided configuration implementations will work. If the node
//! you're talking to is not compatible with the selected configuration, then you'll run into errors
//! (particularly when trying to submit transactions or downloading blocks), and will need to find
//! out what is different between the configuration you've used and the node in question (perhaps it
//! expects things to be signed differently, or has a different address format for instance).
//!
//! ## Usage
//!
//! Once Subxt is configured, the next step is actually interacting with a node. Follow the links
