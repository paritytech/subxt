...
> As a side note, an _extrinsic_ is anything that can be added to a block, and a _transaction_ is an extrinsic that is submitted from a particular user (and is therefore also signed). Subxt can construct unsigned extrinsics, but overwhelmingly you'll need to sign them, and so the documentation tends to use the terms _extrinsic_ and _transaction_ interchangably.
...
> If you're not sure what types to import and use to build a given payload, you can use the `subxt` CLI tool to generate the interface by using something like `subxt codegen | rustfmt > interface.rs`, to see what types and things are available (or even just to use directly instead of the [`#[subxt]`](crate::subxt) macro).

Alternately, we can dynamically construct an extrinsic payload. This will not be type checked or validated until it's submitted:
...
The [`crate::dynamic::Value`] type is a dynamic type much like a `serde_json::Value` but instead represents any type of data that can be SCALE encoded or decoded. It can be serialized, deserialized and parsed from/to strings.

A valid extrinsic payload is just something that implements the [`crate::tx::TxPayload`] trait; you can implement this trait on your own custom types if the built-in ones are not suitable for your needs.

## Signing it

You'll normally need to sign an extrinsic to prove that it originated from an account that you control. To do this, you will typically first create an [`crate::tx::Signer`], which tells Subxt who the extrinsic is from, and takes care of signing the relevant details to prove this.

Subxt provides a [`crate::tx::PairSigner`] which implements this trait (if the `substrate-compat` feature is enabled) which accepts any valid [`sp_core::Pair`] and uses that to sign transactions:
...
If this isn't suitable/available, you can either implement [`crate::tx::Signer`] yourself to use custom signing logic, or you can use some external signing logic, like so:
...
The highest level approach to doing this is to call [`crate::tx::TxClient::sign_and_submit_then_watch_default`]. This hands back a [`crate::tx::TxProgress`] struct which will monitor the transaction status. We can then call [`crate::tx::TxProgress::wait_for_finalized_success()`] to wait for this transaction to make it into a finalized block, check for an `ExtrinsicSuccess` event, and then hand back the events for inspection. This looks like:
...
If you'd like to provide extrinsic parameters (such as mortality), you can use [`crate::tx::TxClient::sign_and_submit_then_watch`] instead, and provide parameters for your chain:
...
This example doesn't wait for the extrinsic to be included in a block; it just submits it and hopes for the best!

### Custom handling of transaction status updates

If you'd like more control or visibility over exactly which status updates are being emitted for the transaction, you can monitor them as they are emitted and react however you choose:
...
Take a look at the API docs for [`crate::tx::TxProgress`], [`crate::tx::TxStatus`] and [`crate::tx::TxInBlock`] for more options.
