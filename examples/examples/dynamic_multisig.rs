...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Now, build an outer call which this inner call will be a part of. In this case,
    // we are setting up a multisig arrangement for the inner call (but we are only
    // specifying one signer; the one we'll use).
    let tx: DynamicTxPayload = subxt::dynamic::tx(
        "Multisig",
        "as_multi",
        vec![
