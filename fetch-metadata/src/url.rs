...
pub async fn get(url: Url, version: MetadataVersion) -> Result<Vec<u8>, Error> {
...
pub fn get_blocking(url: Url, version: MetadataVersion) -> Result<Vec<u8>, Error> {
    tokio_block_on(get(url, version))
}
